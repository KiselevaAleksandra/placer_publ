from intelligent_placer_lib.locationmask import check_by_mask
...
def point_location(approx, x, y):
    ...
    # строим воображаемый луч, параллельный оси у, через точку предмета, которую проверяем
    # смотрим, какие стороны многоугольника пересекает этот луч
    # для пересекаемых сторон смотрим, лежит ли точка предмета над или под прямой, содержащей эту сторону
    # если количество прямых для которых точка лежит ПОД - четное, то точка снуружи, нечетное - внутри
